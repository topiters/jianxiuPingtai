 ...
 * 店铺服务类
 */
class SupperlierModel extends BaseModel {
	...
	public function checkShopName($val,$id = 0){
		$rd = array('status'=>-1);
		$sql = " supperlierName ='%s' and supperlierFlag=1 ";
		$keyArr = array($val);
		if($id>0)$sql.=" and supperlierId!=".$id;
    ...
    /**
	 ...
	 public function insert(){
		...
		//店铺资料
		$sdata = array();
		$sdata["supperlierSn"] = I("supperlierSn");
		$sdata["areaId1"] = (int)I("areaId1");
		$sdata["areaId2"] = (int)I("areaId2");
		$sdata["areaId3"] = (int)I("areaId3");
		$sdata["goodsCatId1"] = (int)I("goodsCatId1");
		$sdata["supperlierName"] = I("supperlierName");
		$sdata["supperlierCompany"] = I("supperlierCompany");
		$sdata["supperlierImg"] = I("supperlierImg");
		$sdata["supperlierAddress"] = I("supperlierAddress");
		...
		$sdata["supperlierTel"] = I("supperlierTel");
		if($this->checkEmpty($data,true) && $this->checkEmpty($sdata,true)){ 
			...
			if(false !== $userId){
				...
				$sdata["supperlierStatus"] = (int)I("supperlierStatus",1);
				$sdata["supperlierAtive"] = (int)I("supperlierAtive",1);
				$sdata["isDistributAll"] = (int)I("isDistributAll",0);
				$sdata["supperlierFlag"] = 1;
				$sdata["createTime"] = date('Y-m-d H:i:s');
			    $sdata['statusRemarks'] = I('statusRemarks');
			    $sdata['qqNo'] = I('qqNo');
			    $sdata["invoiceRemarks"] = I("invoiceRemarks");
				$m = M('supperliers');
				$supperlierId = $m->add($sdata);
				if(false !== $supperlierId){
					$rd['status']= 1;
					//增加商家评分记录
				    $data = array();
				    $data['supperlierId'] = $supperlierId;
				    $m = M('supperlier_scores');
				    $m->add($data);
					//建立店铺和社区的关系
				    if($sdata["isDistributAll"]==0){
						...
						if($relateArea!=''){
							$m = M('supperliers_communitys');
							$relateAreas = explode(',',$relateArea);
							foreach ($relateAreas as $v){
								if($v=='' || $v=='0')continue;
								$tmp = array();
								$tmp['supperlierId'] = $supperlierId;
	 ...
	 public function edit(){
	 	$rd = array('status'=>-1);
	 	$supperlierId = (int)I('id',0);
	 	if($supperlierId==0)return rd;
	 	$m = M('supperliers');
	 	//获取店铺资料
	 	$supperliers = $m->where("supperlierId=".$supperlierId)->find();
	    //检测手机号码是否存在
	 	if(I("userPhone")!=''){
	 		$hasUserPhone = self::checkLoginKey(I("userPhone"),$supperliers['userId']);
	 		if($hasUserPhone==0){
	 			$rd = array('status'=>-2);
	 		    return $rd;
	 		}
	    ...
	    $data = array();
		$data["supperlierSn"] = I("supperlierSn");
		...
		$data["supperlierName"] = I("supperlierName");
		$data["supperlierCompany"] = I("supperlierCompany");
		$data["supperlierImg"] = I("supperlierImg");
		$data["supperlierAddress"] = I("supperlierAddress");
		...
		$data["supperlierStatus"] = (int)I("supperlierStatus",0);
		$data["supperlierAtive"] = (int)I("supperlierAtive",1);
		$isDistributAll = (int)I("isDistributAll",0);
		$data["isDistributAll"] = $isDistributAll;
		$data["supperlierTel"] = I("supperlierTel");
		
		if($this->checkEmpty($data,true)){
			$data['qqNo'] = I('qqNo');
			$data["invoiceRemarks"] = I("invoiceRemarks");
			$rs = $m->where("supperlierId=".$supperlierId)->save($data);
		    if(false !== $rs){
		    	$supperlierMessage = '';
		    	//如果[已通过的店铺]被改为未审核的话也要停止了该店铺的商品
		    	if($supperliers['supperlierStatus']!=$data['supperlierStatus']){
					if($data['supperlierStatus']!=1){
						$sql = "update __PREFIX__goods set isSale=0,goodsStatus=0 where supperlierId=".$supperlierId;
			 	        $m->execute($sql);
			 	        $supperlierMessage = "您的店铺状态已被改为“未审核”状态，如有疑问请与商场管理员联系。";
					}
					if($supperliers['supperlierStatus']!=1 && $data['supperlierStatus']==1){
						$supperlierMessage = "您的店铺状态已被改为“已审核”状态，您可以出售自己的商品啦~";
					}
					$yj_data = array(
						'msgType' => 0,
						'sendUserId' => session('WST_STAFF.staffId'),
						'receiveUserId' => $supperliers['userId'],
						'msgContent' => $supperlierMessage,
		    	...
		    	//检查用户类型
		    	$m = M('users');
		    	$userType = $m->where('userId='.$supperliers['userId'])->getField('userType');
		        ...
		        $urs = $m->where("userId=".$supperliers['userId'])->save($data);
				$rd['status']= 1;
				
				$scm = M('supperliers_communitys');
				$cm = M('communitys');
				if($isDistributAll==0){
			        //建立店铺和社区的关系
					$relateArea = self::formatIn(",", I('relateAreaId'));
					...
					$scm->where('supperlierId='.$supperlierId)->delete();
					if($relateArea!=''){
						$relateAreas = explode(',',$relateArea);
						foreach ($relateAreas as $v){
							if($v=='' || $v=='0')continue;
     ...
     public function get(){
	 	$m = M('supperliers');
		$rs = $m->where("supperlierId=".(int)I('id'))->find();
		$m = M('users');
		...
		//获取店铺社区关系
		$m = M('supperliers_communitys');
		$rc = $m->where('supperlierId='.(int)I('id'))->select();
		$relateArea = array();
	 ...
	 public function reject(){
	 	...
	 	$supperliers = $m->where("supperlierId=".$supperlierId)->find();
	 	$data = array();
	 	$data['supperlierStatus'] = (int)I('supperlierStatus',-1);
			...
			if(false !== $rs){
				...
				if($supperliers['supperlierStatus']!=$data['supperlierStatus']){
					...
					if($data['supperlierStatus']!=1){
						$sql = "update __PREFIX__goods set isSale=0,goodsStatus=0 where supperlierId=".$supperlierId;
			 	        $m->execute($sql);
			 	        if($data['supperlierStatus']==0){
			 	        	$supperlierMessage = "您的店铺状态已被改为“未审核”状态，如有疑问请与商场管理员联系。";
			 	        }else{
			 	        	$supperlierMessage = I('statusRemarks');
			 	        }
					}
					$yj_data = array(
						'msgType' => 0,
     ...
     public function queryByPage(){
        $areaId1 = (int)I('areaId1',0);
	 	...
	 	$sql = "select supperlierId,supperlierSn,supperlierName,u.userName,supperlierAtive,supperlierStatus,gc.catName from __PREFIX__supperliers s,__PREFIX__users u ,__PREFIX__goods_cats gc 
	 	     where gc.catId=s.goodsCatId1 and s.userId=u.userId and supperlierStatus=1 and supperlierFlag=1 ";
	 	if(I('supperlierName')!='')$sql.=" and supperlierName like '%".WSTAddslashes(I('supperlierName'))."%'";
	 	if(I('supperlierSn')!='')$sql.=" and supperlierSn like '%".WSTAddslashes(I('supperlierSn'))."%'";
	 	if($areaId1>0)$sql.=" and areaId1=".$areaId1;
	 	if($areaId2>0)$sql.=" and areaId2=".$areaId2;
	 	$sql.=" order by supperlierId desc";
     ...
     public function queryPeddingByPage(){
        $areaId1 = (int)I('areaId1',0);
     	$areaId2 = (int)I('areaId2',0);
	 	$sql = "select supperlierId,supperlierSn,supperlierName,u.userName,supperlierAtive,supperlierStatus,gc.catName from __PREFIX__supperliers s,__PREFIX__users u ,__PREFIX__goods_cats gc 
	 	     where gc.catId=s.goodsCatId1 and s.userId=u.userId and supperlierStatus<=0 and supperlierFlag=1";
	 	if(I('supperlierName')!='')$sql.=" and supperlierName like '%".WSTAddslashes(I('supperlierName'))."%'";
	 	if(I('supperlierSn')!='')$sql.=" and supperlierSn like '%".WSTAddslashes(I('supperlierSn'))."%'";
	 	if(I('supperlierStatus',-999)!=-999)$sql.=" and supperlierStatus =".(int)I('supperlierStatus');
	 	if($areaId1>0)$sql.=" and areaId1=".$areaId1;
	 	if($areaId2>0)$sql.=" and areaId2=".$areaId2;
	 	$sql.=" order by supperlierId desc";
		return $this->pageQuery($sql);
	  ...
	  */
	  public function queryByList(){
	     $sql = "select * from __PREFIX__supperliers order by supperlierId desc";
		 $rs = $this->find($sql);
	  }
	  ...
	  */
	 public function del(){
	 	$supperlierId = (int)I('id');
	    $rd = array('status'=>-1);
	    //下架所有商品
	    $sql = "update __PREFIX__goods set isSale=0,goodsStatus=-1 where supperlierId=".$supperlierId;
		$this->execute($sql);
		$sql = "select userId from __PREFIX__supperliers where supperlierId=".$supperlierId;
		$supperlier = $this->queryRow($sql);
		//删除登录账号
		$sql = "update __PREFIX__users set userFlag=-1 where userId=".$supperlier['userId'];
		$this->execute($sql);
		...
		//标记店铺删除状态
	    $data = array();
		$data["supperlierFlag"] = -1;
		$data["supperlierStatus"] = -2;
	 	$rs = $this->where("supperlierId=".$supperlierId)->save($data);
	    if(false !== $rs){
			$rd['status']= 1;
		}
		return $rd;
	  ...
	  * 获取待审核的店铺数量
	  */
	 public function queryPenddingSupperlierNum(){
	 	$rd = array('status'=>-1);
	 	$sql="select count(*) counts from __PREFIX__supperliers where supperlierStatus=0 and supperlierFlag=1";
	 	$rs = $this->query($sql);
	 	$rd['num'] = $rs[0]['counts'];
	 	return $rd;
	 }
