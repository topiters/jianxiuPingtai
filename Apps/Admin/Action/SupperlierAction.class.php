...
class SupperlierAction extends BaseAction{
	...
	public function toEdit(){
		...
		//获取商品信息
	    $m = D('Admin/Supperlier');
    	$object = array();
    	if(I('id',0)>0){
    		$this->checkPrivelege('gyslb_02');
    		$object = $m->get();
    	}else{
    		$this->checkPrivelege('gyslb_01');
    		$object = $m->getModel();
    	}
    	
    	$this->assign('object',$object);
    	$this->assign('src',I('src','index'));
		$this->view->display('/supperlier/edit');
	}
   ...
   /**
	...
	public function toView(){
		$this->isLogin();
		$this->checkPrivelege('gyslb_00');
		$m = D('Admin/Supperlier');
		if(I('id')>0){
			$object = $m->get();
			$this->assign('object',$object);
		}
		$this->view->display('/supperlier/view');
	}
	/**
	 * 分页查询
	 */
	...
	public function queryPeddingByPage(){
    	...
    	$this->assign('areaId1',I('areaId1',0));
    	$this->assign('areaId2',I('areaId2',0));
        $this->display("/supperlier/list_pendding");
	}
	/**
	 * 列表查询
	 */
	...
	public function queryPenddingGoodsNum(){
		$this->isLogin();
    	$m = D('Admin/Supperlier');
    	$rs = $m->queryPenddingSupperlierNum();
    	$this->ajaxReturn($rs);
	}
};
?>